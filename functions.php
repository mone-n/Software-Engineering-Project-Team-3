...
if (!function_exists('lp_paid_claim_email_form')) {
	function lp_paid_claim_email_form()
	{
		...
		if (!empty($claim_type) && $claim_type == "paidclaims") {
			...
			$htmlData .= '<input type="email" id="to_claimer_email" name="to_claimer_email" placeholder="' . __('john@gmail.com', 'listingpro') . '" value="' . $claimer_email . '">';
			$htmlData .= '<input type="hidden" id="claimer_id" name="claimer_id" value="' . $claimer . '">';
			$htmlData .= '<br />';
		...
		<div class="lp_all_messages_box clearfix">';
			...
			if (!empty($messages)) {
				...
				foreach ($messages as $key => $singlemessage) {
					...
					$outputcenter .= '<div class="lpQest-image"><img alt="image" src="' . $leadAvatar . '"></div>';
					...
					if (!empty($replymessages)) {
						if (isset($replymessages[$key])) {
							...
							$outputcenter .= '<div class="lpQest-img-outer">';
							$outputcenter .= '<div class="lpQest-image"><img alt="image" src="' . $adminAvatar . '"></div>';
				...
				if (!empty($replymessages)) {
					...
					if ($replySize > $msgCount) {
						...
						for ($i = $msgCount; $i < $replySize; $i++) {
							...
							$outputcenter .= '<p>' . $current_user->user_login . '</p>';
							$outputcenter .= '</div>';
							$outputcenter .= '<div class="lpQestdate"><p>' . date_i18n(get_option("date_format"), strtotime($replytimes[$i])) . '</p></div>';
							$outputcenter .= '</div>';
						}
					}
				}
